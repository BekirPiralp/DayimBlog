...
export class LoginKisimComponent implements OnInit {
  ...
  giris(form: NgForm) {
    this._alertifyService.basarili("girdi");
    this._alertifyService.uyari(
      this.mailPassword.email + " " + this.mailPassword.password
    );

    this._loginService.GirisEpostaSifre = this.mailPassword;
    this._alertifyService.uyari(
      this._loginService.GirisEpostaSifre.email +
        " " +
        this._loginService.GirisEpostaSifre.password
    );
